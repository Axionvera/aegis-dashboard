...
## Interaction With SDK Error Recovery

When a submission fails in a way that may already have applied,
[`buildRecoveryPlan`](sdk-error-recovery.md) sets `reuseIdempotencyKey: true`.
Because the key is derived from the unchanged payload, the retry offered by the
recovery panel naturally resolves to the same key, so a lost-response retry
...
- [SDK Error Recovery Actions](sdk-error-recovery.md)
